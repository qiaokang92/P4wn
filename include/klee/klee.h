...
#undef KLEE_GET_VALUE_PROTO
  ...
  // Hash table APIs
  void klee_table_init(int sz);
  int klee_table_access();
  int klee_table_check();
  int klee_table_read();
  ...
  // Bloom filter APIs
  int klee_bf_init();
  // Note: this call should be renamed bf_read_write
  int klee_bf_access();
  int klee_bf_read();
  ...
  int klee_cmin_init();
  int klee_cmin_add();
  int klee_cmin_larger_than(int x);
  ...
  // Telescope APIs
  int klee_telescope_init(int small_thres, int real_thres);
  int klee_telescope_pkt(int pktID, int reg);
...
#define GREYBOX_MISS    0
#define GREYBOX_HIT     1
#define GREYBOX_COL     2

#define BF_ACCESS_HIT        3
#define BF_ACCESS_MISS_ADD1  4
#define BF_ACCESS_MISS_ADD2  5
#define BF_ACCESS_MISS_ADD3  6

#define MA_READ_TRUE    (1)
#define MA_READ_FALSE   (0)

#define HT_READ_TRUE    MA_READ_TRUE
#define HT_READ_FALSE   MA_READ_FALSE

#define POISE_DEC_INVALID   (-1)
#define POISE_DEC_ALLOW     MA_READ_TRUE
#define POISE_DEC_DENY      MA_READ_FALSE

#define NW_IPD_LARGE  MA_READ_TRUE
#define NW_IPD_SMALL  MA_READ_FALSE

#define POISE_PORT_INVALID  (-1)
#define POISE_PORT_ALLOW    (0)
#define POISE_PORT_DENY     (1)
#define POISE_PORT_CTX      (2)
