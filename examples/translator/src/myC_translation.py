...
def run(node, rules, profileProgram, iterN):
    global inputProfileProgram, iterNum
    cleanup_variables()
    if rules != None:
        global forwardingRules
        forwardingRules = rules
    inputProfileProgram = profileProgram
    iterNum = iterN
...
def Declaration_Instance(node):
    ...
    elif hasattr(node.type.baseType, "path"):
        if hasattr(node.type.baseType.path, "name"):
            # There is a register!
            if node.type.baseType.path.name == "register":
                regName = node.name
                regBits = node.type.arguments.vec[0].size                
                regCount = node.arguments.vec[0].value
                print "register Name: "+regName
                print "register Bits: "+str(regBits)
                print "register Count: "+str(regCount)
                arrDef = bitsSizeToType(regBits)+" "+regName+"["+str(regCount)+"];"
                print arrDef
                returnString += arrDef
...
def MethodCallExpression(node):
    ...
    # read register, TODO: separate this into an 'extern methods' method
        ...
        # print "TOC; "+toC(node.arguments.vec[rtnVarNum])
        
        # if node.arguments.vec[indexNum].Node_Type == "Constant":
        #     index = Constant(node.arguments.vec[indexNum])
        # elif node.arguments.vec[indexNum].Node_Type == "PathExpression":
        #     index = PathExpression(node.arguments.vec[indexNum])
        # elif node.arguments.vec[indexNum].Node_Type == "Cast":
        #     index = Cast(node.arguments.vec[indexNum])
        
        # if node.arguments.vec[rtnVarNum].Node_Type == "Constant":
        #     rtnVar = Constant(node.arguments.vec[rtnVarNum])
        # elif node.arguments.vec[rtnVarNum].Node_Type == "PathExpression":
        #     rtnVar = PathExpression(node.arguments.vec[rtnVarNum])
        # elif node.arguments.vec[rtnVarNum].Node_Type == "Cast":
        #     rtnVar = Cast(node.arguments.vec[rtnVarNum])
        
        arrDef = node.method.expr.path.name
        if len(node.arguments.vec) > 1:
            index = toC(node.arguments.vec[1])
            rtnVar = toC(node.arguments.vec[0])

            readStr = rtnVar + " = "+arrDef+"["+index+"];"
            print readStr
            returnString += readStr
        else:
            print "too short"
            print "len: "+str(len(node.arguments.vec))
            print toC(node.arguments.vec[0])
            print arrDef
            returnString += "// a meter read here, meter_index.read(meta.meter_metadata.packet_color); \n"
    ...
    # write register, TODO: separate this into an 'extern methods' method
        ...
        index = toC(node.arguments.vec[0])
        rtnVar = toC(node.arguments.vec[1])

        writeStr = arrDef+"["+index+"] = " + rtnVar+";"
        print writeStr
        returnString += writeStr
...
def P4Parser(node):
    ...
    if inputProfileProgram != 0:
        returnAssumeString, returnProbString = ParseHeaderDistribution(inputProfileProgram)
        returnString += returnAssumeString
        returnString += returnProbString    
    
    returnString += "\tstart();\n}\n"
    return returnString 

def ParseHeaderDistribution(node):    
    print "ParseHeaderDistribution:"    
    returnAssumeString = ""
    returnProbString = ""
    for i in range(len(node.headers)):
        headerName = node.headers[i].header_name
        headerRanges = node.headers[i].ranges
        if i != 0:
            returnAssumeString += "\n\t" 
        for j in range(len(headerRanges)):
            rangeLow = headerRanges[j].range_low
            rangeHigh = headerRanges[j].range_high
            rangeProb = headerRanges[j].prob
            returnAssumeString += "("+headerName+" >= "+str(rangeLow)+" && "+headerName+" <= "+str(rangeHigh)+") || "
            returnProbString += "klee_prob(\""+headerName+"\", "+str(rangeLow+1)+", "+str(rangeHigh+1)+", "+str(rangeProb)+");\n"

    returnAssumeString = "klee_assume("+returnAssumeString[:len(returnAssumeString)-4]+");\n"
    
    #print returnAssumeString
    #print returnProbString

    return returnAssumeString, returnProbString
...
def SymbolizeParameters(node):
    ...
    for param in node.type.applyParams.parameters.vec:
        ...
        if (param.direction == "out" or param.direction == "inout") and param.type.Node_Type == 'Type_Name':
            if param.type.path.name == "headers": # hdr
                returnString += klee_make_symbolic(param.name)
            else: # metadata or standard_metadata
                returnString += "// "+klee_make_symbolic(param.name)
