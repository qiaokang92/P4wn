...
class Executor : public Interpreter {
  ...
  friend class HashTableHandler;
  friend class TeleScopeHandler;
  friend class HashTable;
  friend class BFHandler;
  friend class BF;
  friend class CminHandler;
  friend class CminSketch;
  friend class MAHandler;
  friend class MATable;
  friend class SseHandler;
...
public:
  ...
  // TODO: define virtual API for counter
  LattECounter *counter;

  SseHandler *sseHandler;

  // hash table handler pointer
  HashTableHandler *htHandler;

  // BF handler pointer
  BFHandler *bfHandler;

  // Cmin handler pointer
  CminHandler *cminHandler;

  // Cmin handler pointer
  MAHandler *maHandler;

  // Query handler pointer
  QueryHandler *queryHandler;

  // Telescope handler pointer
  TeleScopeHandler *tsHandler;

  std::map<unsigned, BranchProb *> cond2bp;
...
private:
  ...
  //void printPerPathProbs();
  void processPathProbs();
  ...
  StatePair fork(ExecutionState &current, ref<Expr> condition, bool isInternal,
                 int line=0);
                 //std::string progName="", int line=0);
  //StatePair fork(ExecutionState &current, ref<Expr> condition, bool isInternal);

  // APIs for table access
  StatePair forkTableAccess(ExecutionState &current);

  void branchTableAccess(ExecutionState &state,
                         const std::vector< ref<Expr> > &conditions,
                         std::vector<ExecutionState*> &result);

  void branchTableAccessNoCond(ExecutionState &state, int N,
                               std::vector<ExecutionState*> &result);
