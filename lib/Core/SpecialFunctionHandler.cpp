...
#define addDNR(name, handler) { name, \
  ...
  add("klee_mytest", handleTest, false),

  // hash table APIs
  add("klee_table_init", handleTableInit, false),
  add("klee_table_access", handleTableAccess, true),
  add("klee_table_check", handleTableCheck, true),
  ...
  // BF APIs
  add("klee_bf_init", handleBFInit, false),
  add("klee_bf_access", handleBFAccess, true),
  add("klee_bf_read", handleBFRead, true),

  // Cmin APIs
  add("klee_cmin_init", handleCminInit, false),
  add("klee_cmin_add", handleCminAdd, false),
  add("klee_cmin_larger_than", handleCminLargerThan, true),

  // MA table APIs
  add("klee_ma_init",   handleMAInit,   false),
  ...
  // Telescope APIs
  add("klee_telescope_init", handleTSIinit, false),
  add("klee_telescope_pkt", handleTSPkt, false),
  add("klee_telescope", handleTS, false),
  add("klee_telescope_cmin_pkt", handleTSCminPkt, false),
  add("klee_telescope_cmin", handleTSCmin, false),

  // Utils
  add("klee_update_iter", handleUpdateIter, false),
...
void SpecialFunctionHandler::handleTest(ExecutionState &state,
                                        KInstruction *target,
                                        std::vector<ref<Expr> > &arguments)
{
   klee_message("arrived at handleTest!!");
}

void SpecialFunctionHandler::handleTableInit(ExecutionState &state,
                                             KInstruction *target,
                                             std::vector<ref<Expr> > &arguments)
{
   int ret;
   ...
   size = cast<ConstantExpr>(arguments[0])->getZExtValue();

   ret = executor.htHandler->ht_init(state, size);
   assert(ret == 0);
}
                                               ...
                                               std::vector<ref<Expr> > &arguments)
{
   ...
   int ret = executor.htHandler->ht_access(state, states, rets);
...
void SpecialFunctionHandler::handleTableCheck(ExecutionState &state,
                                              KInstruction *target,
                                              std::vector<ref<Expr> > &arguments)
{
   int status;
   ...
   ret = executor.htHandler->ht_check(state, status);
   assert(ret == 0);

   // return the status of the last klee_table_access call
   ref<Expr> e = ConstantExpr::create(status, Expr::Int32);
   // e.get()->dump();
   executor.bindLocal(target, state, e);
}

void SpecialFunctionHandler::handleTableRead(ExecutionState &state,
                                             KInstruction *target,
                                             std::vector<ref<Expr> > &arguments)
{
   int ret;
   ...
   ret = executor.htHandler->ht_read(state, states, rets);
   assert(ret == 0);
...
void SpecialFunctionHandler::handleTableWrite(ExecutionState &state,
                                              KInstruction *target,
                                              std::vector<ref<Expr> > &arguments)
{
   // no return value.
   int val = cast<ConstantExpr>(arguments[0])->getZExtValue();
   int ret;

   ret = executor.htHandler->ht_write(state, val);
   assert(ret == 0);
}

void SpecialFunctionHandler::handleTableAdd(ExecutionState &state,
                                            KInstruction *target,
                                            std::vector<ref<Expr> > &arguments)
{
   // no return value.
   ...
   ret = executor.htHandler->ht_add_handler(state);
   assert(ret == 0);
}

void SpecialFunctionHandler::handleReadLargerThan(ExecutionState &state,
                                                  KInstruction *target,
                                         std::vector<ref<Expr> > &arguments)
{
   // no return value.
   ...
   ret = executor.htHandler->ht_read_larger_than(state, val, states, rets);
   assert(ret == 0);
   assert(states.size() == rets.size());
...
void SpecialFunctionHandler::handleTestSum(ExecutionState &state,
                                              KInstruction *target,
                                              std::vector<ref<Expr> > &arguments)
{
   // no return value.
   int sum = cast<ConstantExpr>(arguments[0])->getZExtValue();
   int ret;
   vector<ExecutionState*> states;
   vector<int> rets;

   ret = executor.htHandler->ht_test_sum(state, sum, states, rets);
   assert(ret == 0);
   assert(states.size() == rets.size());
...
// BF API handlers
void SpecialFunctionHandler::handleBFInit(ExecutionState &state,
                                         KInstruction *target,
                                         std::vector<ref<Expr> > &arguments)
{
   int ret;
   ...
   ret = executor.bfHandler->bf_init(state, size);
   assert(ret == 0);
}

void SpecialFunctionHandler::handleBFAccess(ExecutionState &state, KInstruction *target,
                                          std::vector<ref<Expr> > &arguments)
{
   vector<ExecutionState*> states;
   vector<int> rets;

   int ret = executor.bfHandler->bf_access(state, states, rets);
...
void SpecialFunctionHandler::handleBFRead(ExecutionState &state, KInstruction *target,
                                          std::vector<ref<Expr> > &arguments)
{
   vector<ExecutionState*> states;
   vector<int> rets;

   int ret = executor.bfHandler->bf_read(state, states, rets);
...
// Cmin API handlers
void SpecialFunctionHandler::handleCminInit(ExecutionState &state,
                                         KInstruction *target,
                                         std::vector<ref<Expr> > &arguments)
{
   int ret;
   ...
   ret = executor.cminHandler->cmin_init(state, size);
   assert(ret == 0);
}

void SpecialFunctionHandler::handleCminAdd(ExecutionState &state,
                                           KInstruction *target,
                                           std::vector<ref<Expr> > &arguments)
{
   vector<ExecutionState*> states;
   ...
   int ret = executor.cminHandler->cmin_add(state, states, rets);

   assert(states.size() == rets.size());
   assert(ret == 0);
}

void SpecialFunctionHandler::handleCminLargerThan(ExecutionState &state,
                                          KInstruction *target,
                                          std::vector<ref<Expr> > &arguments)
{
   int val = cast<ConstantExpr>(arguments[0])->getZExtValue();
   ...
   int ret = executor.cminHandler->cmin_larger_than(state, val, states, rets);
...
// MA table API handlers
void SpecialFunctionHandler::handleMAInit(ExecutionState &state,
                                         KInstruction *target,
                                         std::vector<ref<Expr> > &arguments)
{
   int ret;
   ret = executor.maHandler->ma_init(state);
   assert(ret == 0);
}

void SpecialFunctionHandler::handleMAAccess(ExecutionState &state,
                                            KInstruction *target,
                                            std::vector<ref<Expr> > &arguments)
{
   vector<ExecutionState*> states;
   ...
   int ret = executor.maHandler->ma_access(state, states, rets);
...
void SpecialFunctionHandler::handleMARead(ExecutionState &state,
                                          KInstruction *target,
                                          std::vector<ref<Expr> > &arguments)
{
   vector<ExecutionState*> states;
   ...
   int ret = executor.maHandler->ma_read(state, states, rets);
...
void SpecialFunctionHandler::handleMAInsert(ExecutionState &state,
                                            KInstruction *target,
                                            std::vector<ref<Expr> > &arguments)
{
   vector<ExecutionState*> states;
   ...
   int ret = executor.maHandler->ma_insert(state, val);
...
/*
void SpecialFunctionHandler::handleQuery(ExecutionState &state,
                                         KInstruction *target,
                                         std::vector<ref<Expr> > &arguments)
{
   vector<ExecutionState*> states;
   ...
   int query = cast<ConstantExpr>(arguments[0])->getZExtValue();

   //int ret = executor.queryHandler->handleQuery(query);
...
void SpecialFunctionHandler::handleTSIinit(ExecutionState &state,
                                           KInstruction *target,
                                           std::vector<ref<Expr> > &arguments)
{
   int small = cast<ConstantExpr>(arguments[0])->getZExtValue();
   int real  = cast<ConstantExpr>(arguments[1])->getZExtValue();

   int ret = executor.tsHandler->ts_init_handler(&state, small, real);
   assert(ret == 0);
}

void SpecialFunctionHandler::handleTSPkt(ExecutionState &state,
                                         KInstruction *target,
                                         std::vector<ref<Expr> > &arguments)
{
   int id   = cast<ConstantExpr>(arguments[0])->getZExtValue();
   int reg  = cast<ConstantExpr>(arguments[1])->getZExtValue();

   int ret = executor.tsHandler->ts_pkt_handler(&state, id, reg);
   assert(ret == 0);
}

void SpecialFunctionHandler::handleTS(ExecutionState &state,
                                           KInstruction *target,
                                           std::vector<ref<Expr> > &arguments)
{
   int reg = cast<ConstantExpr>(arguments[0])->getZExtValue();
   int ret = executor.tsHandler->ts_handler(&state, reg);
   assert(ret == 0);
}

void SpecialFunctionHandler::handleUpdateIter(ExecutionState &state,
                                              KInstruction *target,
                                              std::vector<ref<Expr> > &arguments)
{
   int i = cast<ConstantExpr>(arguments[0])->getZExtValue();
   state.updateIter(i);
}

void SpecialFunctionHandler::handleTSCminPkt(ExecutionState &state,
                                             KInstruction *target,
                                             std::vector<ref<Expr> > &arguments)
{
   int id = cast<ConstantExpr>(arguments[0])->getZExtValue();
   int ret = executor.tsHandler->ts_cmin_pkt_handler(&state, id);
   assert(ret == 0);
}

void SpecialFunctionHandler::handleTSCmin(ExecutionState &state,
                                          KInstruction *target,
                                          std::vector<ref<Expr> > &arguments)
{
