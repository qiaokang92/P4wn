...
cl::opt<unsigned> FullPse(
    "full-pse", cl::init(1),
    cl::desc("true: run full PSE, false: run SSE "));

cl::opt<unsigned> IsEnabled(
    "enable-is", cl::init(0),
    cl::desc("true: enable IS when running pse, false: naive PSE"));

cl::opt<double> SseConfidence(
    "sse-confidence", cl::init(0.95),
    cl::desc("the confidence level of Bayes Inference"));

cl::opt<double> SseInterval(
    "sse-interval", cl::init(0.01),
    cl::desc("the interval, i.e., epsilon"));

cl::opt<unsigned> TargetLine(
    "target-line", cl::init(31),
    cl::desc("which code block you are interested in"));

cl::opt<unsigned> maxNumSample(
    "max-sample", cl::init(100),
    cl::desc("max number of paths to sample"));

cl::opt<unsigned> ProgNo(
    "program-num",
    cl::desc("program # to be executed"));

cl::opt<unsigned> QueryRealTrace(
    "real-trace", cl::init(0),
    cl::desc("if query real trace, 1 to enable"));

cl::opt<unsigned> MaxNumTracePkt(
    "num-trace-pkt", cl::init(10000),
    cl::desc("max number of pkts to query"));

cl::opt<unsigned> NumLoop(
    "num-loop", cl::init(1),
    cl::desc("num of loop iterations for stateful program"));

cl::opt<unsigned> Profile(
    "profile", cl::init(1),
    cl::desc("num of traffic profile"));

cl::opt<unsigned> EnableTS(
    "enable-ts", cl::init(0),
    cl::desc("1 to enable telescoping"));

cl::opt<unsigned> TSThres(
    "ts-thres", cl::init(0),
    cl::desc("The threshold of telescoping"));

cl::opt<double> ConvergeDelta(
    "converge-delta", cl::init(1e-10),
    cl::desc("The delta for convergence test"));
...
static void printAllConstraints(ConstraintManager *cons)
{
   // this constraints set includes the constraints added by klee_assume
   typedef std::vector< ref<Expr> >::const_iterator constraint_iterator;
   klee_message("check original constraints collected so far:");
   ...
   klee_message("check original constraints done:");
}

Executor::Executor(LLVMContext &ctx, const InterpreterOptions &opts,
                   InterpreterHandler *ih):
      Interpreter(opts),
      allpbs(0), counter(0), sseHandler(0), htHandler(0),
      bfHandler(0), cminHandler(0), maHandler(0),
      queryHandler(0),
      tsHandler(new TeleScopeHandler(NumLoop)),
      interpreterHandler(ih), searcher(0),
...
{
  std::string progName = queryHandler->getProgNameFromNumer(ProgNo);
  this->allpbs = new AllPathProbs(NumLoop, progName, ProgNo, Profile, ConvergeDelta);

  this->counter = new LattECounter();

  // initialize hash table pointer to executor
  htHandler = new HashTableHandler(*this);
  bfHandler = new BFHandler(*this);
  cminHandler = new CminHandler(*this);
  maHandler = new MAHandler(*this);
  if (QueryRealTrace) {
     klee_warning("real trace enabled");
  } else {
     klee_warning("real trace disabled");
  }
  queryHandler = new QueryHandler(ProgNo, QueryRealTrace, MaxNumTracePkt,
                                  Profile, this);

  if (FullPse == 0) {
     this->sseHandler= new SseHandler(this, TargetLine,
                                      SseConfidence, SseInterval, maxNumSample,
                                      IsEnabled);
  }
               ...
               int line)
{
  ...
  if (isSeeding &&
    ...
    if (!(trueSeed && falseSeed)) {
      ...
      addConstraint(current, trueSeed ? condition :
  ...
  if (res==Solver::True) {
    ...
    // if we are doing sampling, do not fork new states
    if (FullPse == 0) {
       assert(current.isSample == true);
       assert(current.isSampleDone == false);

       double r, ifProb;
       r = theRNG.getDouble();
       ifProb = queryHandler->queryProbByLine(line, condition);

       if (r < ifProb) {
          LOG(LOG_MASK_EXE, "taking the if branch with %f", ifProb);
          addConstraint(current, condition);
          return StatePair(&current, 0);
       } else {
          LOG(LOG_MASK_EXE, "taking the else branch with %f", 1-ifProb);
          addConstraint(current, Expr::createIsZero(condition));
          return StatePair(0, &current);
       }
    }
    ...
    if (pathWriter) {
      // Need to update the pathOS.id field of falseState,
...
void Executor::branchTableAccessNoCond(ExecutionState &state, int N,
                                       std::vector<ExecutionState*> &result)
{
   if (FullPse == 0) {
     assert(state.isSample == true);
     assert(state.isSampleDone == false);
     assert(0 && "branch should never happen for Monte Carlo sampling");
   }

   assert(N);
   stats::forks += N-1;

   // the current state is the first one
   result.push_back(&state);
...
void Executor::branchTableAccess(ExecutionState &state,
                                 const std::vector< ref<Expr> > &conditions,
                                 std::vector<ExecutionState*> &result)
{
   unsigned N = conditions.size();

   branchTableAccessNoCond(state, N, result);
...
*/
   ...
   // add constraints for each forked states
   for (unsigned i=0; i<N; ++i) {
      if (result[i]) {
         addConstraint(*result[i], conditions[i]);
      }
...
Executor::StatePair
Executor::forkTableAccess(ExecutionState &current)
{
   ...
   current.ptreeNode->data = 0;
   std::pair<PTree::Node*, PTree::Node*> resTree =
         processTree->split(current.ptreeNode, falseState, trueState);
   falseState->ptreeNode = resTree.first;
   trueState->ptreeNode = resTree.second;

   // Kinda gross, do we even really still want this option?
   if (MaxDepth && MaxDepth<=trueState->depth) {
     assert(!"maxdepth exceeded!");
     terminateStateEarly(*trueState, "max-depth exceeded.");
     terminateStateEarly(*falseState, "max-depth exceeded.");
     return StatePair(0, 0);
   }
                           ...
                           ExecutionState &state) const
{
  ...
  if (vnumber < 0) {
    ...
    //klee_message("not a constant, returning %s", sf.locals[index].value);
...
void Executor::executeCall(ExecutionState &state,
  ...
  if (f && f->isDeclaration()) {
    //klee_message("this is f && f->isDeclaration()");
    switch(f->getIntrinsicID()) {
    case Intrinsic::not_intrinsic:
      // klee_message("not_intrinsic");
    ...
    // FIXME: I'm not really happy about this reliance on prevPC
    ...
    if (!f->isVarArg()) {
      ...
      for (unsigned i = funcArgs; i < callingArgs; i++) {
        ...
        if (WordSize == Expr::Int32) {
          ...
          // AMD64-ABI 3.5.7p5:
          // Step 7. Align l->overflow_arg_area upwards to a
      ...
      if (mo) {
        ...
        unsigned offset = 0;
        for (unsigned i = funcArgs; i < callingArgs; i++) {
          // FIXME: This is really specific to the architecture,
          // not the pointer
          // size. This happens to work for x86-32 and x86-64, however.
          if (WordSize == Expr::Int32) {
...
void Executor::processPathProbs()
{
   allpbs->processAccumPkt();
   allpbs->processRanking();
   allpbs->dumpFinalRanking();
   allpbs->dumpPerPktRanking();

   if (allpbs->isStable("ranking")) {
      LOG(-1, "ranking converged!");
   } else {
      LOG(-1, "ranking not converged..");
   }

   if (allpbs->isStable("delta")) {
      LOG(-1, "delta converged!");
   } else {
      LOG(-1, "delta not converged..");
   }
}


// dump per-path probabilities
/*
void Executor::printPerPathProbs()
{
   for (std::vector<BranchProb *>::iterator it =allpbs->pbs.begin(),
                                            ie =allpbs->pbs.end();
        it != ie; ++it) {
      LOG(LOG_MASK_ALL, "branch id=%d, line=%d, ifAccumProb=%g, elseAccumProb=%g",
          (*it)->id, (*it)->line, (*it)->ifAccumProb, (*it)->elseAccumProb);
   }
}
*/
...
void Executor::executeInstruction(ExecutionState &state, KInstruction *ki)
{
  ...
  //LOG(LOG_MASK_SSE, "state prob is %f, opcode is %d", state.getPathProb(),
  //    i->getOpcode());
  ...
  case Instruction::Ret: {
    ...
    if (state.stack.size() <= 1) {
      ...
      // klee_message("state returning from main function");
  ...
  case Instruction::Br: {
    ...
    // cond for if to be true
    cond = optimizer.optimizeExpr(cond, false);

    //printAllConstraints(&state.constraints);
    // WARN("dumping cond at line %d", line);
    //(*cond.get()).dump();

    double currProb = state.getPathProb();
    double ifProb = 0.0;
    BranchProb *bp;
    bool isProb = cond.get()->getKind() != Expr::Constant;
    unsigned line = ki->info->line;

    LOG(LOG_MASK_EXE, "state[%d]: arrived at a %s branch, prob=%g, line=%d",
        state.id, isProb ? "probabilistic" : "determinstic",
        currProb, line);

    // find the BranchProb instance
    std::map<unsigned, BranchProb *>::iterator it;
    it = cond2bp.find(line);
    if (it == cond2bp.end()) {
       // not found, create a new BranchProb instance
       bp = new BranchProb(state.depth, line);
       cond2bp[line] = bp;
       allpbs->pbs.push_back(bp);
    } else {
       bp = it->second;
    }

    // fork and obtain (at most) two execution state
    Executor::StatePair branches = fork(state, cond, false, line);

    // count how many times this state triggers the if branch
    if (line == TargetLine && state.isSample && branches.first) {
       assert(state.isSampleDone == false);
       state.sampleRes += 1;
    }

    if (bp->ifAccumPkt.find(iter) == bp->ifAccumPkt.end()) {
       bp->ifAccumPkt[iter] = 0;
    }
    if (bp->elseAccumPkt.find(iter) == bp->elseAccumPkt.end()) {
       bp->elseAccumPkt[iter] = 0;
    }

    if (isProb) {
       // This is a full PSE run
       if (FullPse == 1) {
          if (branches.first && branches.second) {

             // let's compute time on this operation
             ifProb = queryHandler->queryProbByLine(line, cond);
             LOG(LOG_MASK_EXE, "if branch prob is %f", ifProb);
          } else if (branches.first) {
             ifProb = 1;
             LOG(LOG_MASK_EXE, "only if branch is possible");
          } else {
             ifProb = 0;
             LOG(LOG_MASK_EXE, "only else branch is possible");
          }
       } else {
          // In SSE run, we don't need t update AccumProbs, so we don't
          // have to compute the right ifProb, so assign some random value
          // but we still need to update path prob
          // ifProb = 0;
          ifProb = queryHandler->queryProbByLine(line, cond);
       }

       if (branches.first) {
          branches.first->updatePathProb(ifProb);
          bp->ifAccumProb += branches.first->getPathProb();
          bp->ifAccumPkt[iter] += branches.first->getPathProb();
       }

       if (branches.second) {
          branches.second->updatePathProb(1 - ifProb);
          bp->elseAccumProb += branches.second->getPathProb();
          bp->elseAccumPkt[iter] += branches.second->getPathProb();
       }
    } else {
       std::string condStr = counter->expr2Str(cond);
       if (condStr == "true") {
          LOG(LOG_MASK_EXE, "line %d ifProb updated %g -> %g",
              line, bp->ifAccumProb, bp->ifAccumProb + currProb);
          bp->ifAccumProb += currProb;
          bp->ifAccumPkt[iter] += currProb;
       } else if (condStr == "false") {
          LOG(LOG_MASK_EXE, "line %d elseProb updated %g -> %g",
              line, bp->elseAccumProb, bp->elseAccumProb + currProb);
          bp->elseAccumProb += currProb;
          bp->elseAccumPkt[iter] += currProb;
       } else {
          klee_message("condStr is %s", condStr.c_str());
          assert(!"unrecogized condition cond string!");
       }
    }
  ...
  case Instruction::Call: {
    ...
    // klee_message("calling function  %s",
    //              f->getName().str().c_str());
    ...
    if (f) {
      //klee_message("this function exists");
...
void Executor::run(ExecutionState &initialState)
{
  ...
  // construct a searcher class, we will need to modify this to add our
  // heuristic function.
...
#define MAX_STATE_SIZE 32
  ...
  // If it is an SSE run
  if (FullPse == 0) {
     // pick up the initial state
     assert(states.size() == 1);
     ExecutionState &state = searcher->selectState();

     LOG(LOG_MASK_EXE, "it is an SSE run, IS is %s",
         IsEnabled? "enabled":"disabled");
     assert(sseHandler);

     // invoke the SSE handler
     sseHandler->run(state);
  ...
  LOG(-1, "it is a full PSE run");

  if (EnableTS) {
     LOG(-1, "Telescope is enabled in this run");
  }

  // entry of the main loop
  while (!states.empty() && !haltExecution) {
    // klee_message("We have %ld states to run!!", states.size());
    ExecutionState &state = searcher->selectState();
    KInstruction *ki = state.pc;

    // Move to and execute the next instruction
    stepInstruction(state);
    executeInstruction(state, ki);
  ...
  if (EnableTS) {
     tsHandler->ProcessAllPaths(std::vector<ExecutionState *>());
  } else {
     processPathProbs();
  }

  LOG(-1, "numQueries %d, numQueriesNotFound %d,"
      " numQueries per pkt %d, queryTime %g s "
      "time per query %g s",
      queryHandler->numQueries,
      queryHandler->numQueriesNotFound,
      queryHandler->numQueries / NumLoop,
      (double)queryHandler->queryTime/1000000.0,
      (double)queryHandler->queryTime/1000000.0/queryHandler->numQueries);
...
void Executor::evictStates(int num)
{
   int i = 0;
   ...
   for (; i < num; i ++) {
      ExecutionState *s = *select_random(states, indices[i]);
      terminateState(*s);
   }
}
...
void Executor::terminateStateOnExit(ExecutionState &state)
{
  ...
  if (!OnlyOutputStatesCoveringNew || state.coveredNew ||
     ...
     // In profiling phase, we don't need test cases
  ...
  if (state.isSample) {
     LOG(LOG_MASK_SSE, "one state exited, setting SSE flag as done");
     state.isSampleDone = true;
  }
}
...
void Executor::callExternalFunction(ExecutionState &state,
  ...
  bool success = externalDispatcher->executeCall(function,
                                                 target->inst, args);
  if (!success) {
    terminateStateOnError(state, "failed external call: " +
                          function->getName(),
                          External);
    return;
  }
...
void Executor::runFunctionAsMain(Function *f,
  ...
  if (ai!=ae) {
    klee_message("we are in ai!=ae branch");
  ...
  LOG(LOG_MASK_EXE, "starting a new state for function %s",
      f->getName().str().c_str());
  ...
  // We don't enter this branch for the simplest prog test
  if (argvMO) {
    klee_message("in argvMO branch");
  ...
  // initialize a process tree
  processTree = new PTree(state);
  state->ptreeNode = processTree->root;

  // start running from the root state
  run(*state);
  ...
  klee_message("leaving runFunctionAsMain");
}
